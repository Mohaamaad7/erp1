...
class AdvancedInventoryCatalogManager
{
	...
	public function getProductsWithInventoryInfo($filters = array(), $sortfield = 'p.ref', $sortorder = 'ASC', $limit = 0, $offset = 0)
	{
		...
		if ($this->checkSupplierTable()) {
			$sql .= ", supplier_count.supplier_count, ds.supplier_name as default_supplier_name, ds.supplier_part_num as default_part_num";
		} else {
		...
		if (!empty($filters['search_ref'])) {
			$sql .= " AND p.ref LIKE '%".$this->db->escape($filters['search_ref'])."%'";
		}
		if (!empty($filters['search_label'])) {
			$sql .= " AND p.label LIKE '%".$this->db->escape($filters['search_label'])."%'";
		}
		if (!empty($filters['search_smart_code']) && $this->checkExtrafieldsTable()) {
			$sql .= " AND pe.advinv_smart_code LIKE '%".$this->db->escape($filters['search_smart_code'])."%'";
		}
		if (!empty($filters['search_part_number']) && $this->checkSupplierTable()) {
			$sql .= " AND EXISTS (SELECT 1 FROM ".MAIN_DB_PREFIX."advancedinventory_supplier_item si3 WHERE si3.fk_product = p.rowid AND si3.supplier_part_num LIKE '%".$this->db->escape($filters['search_part_number'])."%')";
		}
		if (isset($filters['type']) && $filters['type'] != '-1' && $filters['type'] !== '') {
			$sql .= " AND p.fk_product_type = ".((int) $filters['type']);
		}
		if (isset($filters['tosell']) && $filters['tosell'] != '-1' && $filters['tosell'] !== '') {
			$sql .= " AND p.tosell = ".((int) $filters['tosell']);
		}
		if (isset($filters['tobuy']) && $filters['tobuy'] != '-1' && $filters['tobuy'] !== '') {
			$sql .= " AND p.tobuy = ".((int) $filters['tobuy']);
		}
		if (!empty($filters['reorder_alert']) && $this->checkExtrafieldsTable()) {
			$sql .= " AND pe.advinv_reorder_point > 0";
			if ($this->checkStockTable()) {
				$sql .= " AND COALESCE(stock_total.total_stock, 0) <= pe.advinv_reorder_point";
			}
		}
		if (!empty($filters['supplier_id']) && $this->checkSupplierTable()) {
	...
	public function getProductsCount($filters = array())
	{
		...
		if (!empty($filters['search_part_number']) && $this->checkSupplierTable()) {
			$sql .= " AND EXISTS (SELECT 1 FROM ".MAIN_DB_PREFIX."advancedinventory_supplier_item si WHERE si.fk_product = p.rowid AND si.supplier_part_num LIKE '%".$this->db->escape($filters['search_part_number'])."%')";
		}
		if (isset($filters['type']) && $filters['type'] != '-1' && $filters['type'] !== '') {
			$sql .= " AND p.fk_product_type = ".((int) $filters['type']);
		}
	...
	public function getCatalogStatistics()
	{
		...
		// Products below reorder point (simplified for now)
		$stats['products_below_reorder'] = 0;
