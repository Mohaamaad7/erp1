...
if (empty($reshook)) {
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		$object->fk_parent = GETPOST('fk_parent', 'int') ? GETPOST('fk_parent', 'int') : null;
...
if ($action == 'create') {
	...
	$temp_ref = '';
	if (empty($temp_ref)) {
		// Create temporary object to get next ref
		$temp_warehouse = new AdvancedInventoryWarehouse($db);
		$temp_ref = $temp_warehouse->getNextNumRef();
	}
	print '<td>'.$temp_ref.' ('.$langs->trans("AutoGenerated").')</td></tr>';
