...
/**
 * \file    advancedinventory/admin/setup.php
 * \ingroup advancedinventory
 * \brief   Advancedinventory setup page.
 */
...
require_once '../lib/advancedinventory.lib.php';
...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('advancedinventorysetup', 'globalsetup'));
...
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM1');
$item->fieldParams['isMandatory'] = 1;
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';
$item->helpText = 'Tooltip text';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('ADVANCEDINVENTORY_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('ADVANCEDINVENTORY_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('ADVANCEDINVENTORY_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('ADVANCEDINVENTORY_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('ADVANCEDINVENTORY_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('ADVANCEDINVENTORY_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('ADVANCEDINVENTORY_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf ADVANCEDINVENTORY_MYPARAM10
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('ADVANCEDINVENTORY_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/advancedinventory/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=advancedinventory-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'ADVANCEDINVENTORY_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'ADVANCEDINVENTORY_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "AdvancedinventorySetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-advancedinventory page-admin');
...
$head = advancedinventoryAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "advancedinventory@advancedinventory");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("AdvancedinventorySetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'ADVANCEDINVENTORY_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'ADVANCEDINVENTORY_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
